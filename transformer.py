...
class SelfAttention(nn.Module):
    def __init__(self, embed_size, heads):
        ...
        self.output  = nn.Linear(heads * self.head_dim, embed_size)
    ...
    def forward(self, values, keys, queries, mask):
        ...
        attention = torch.softmax(energy / (self.embed_size ** (1/2)), dum = 3)
...
class Transformer(nn.Module):
    def __init__(self, src_vocab_size, trg_vocab_size, src_pad_idx, trg_pad_idx, 
                ...
                dropout = 0, device = 'cpu', max_length = 100) -> None:
        ...
        self.src_pas_idx = src_pad_idx
        self.trg_pad_idx = trg_pad_idx

        self.device = device 

    def make_source_mask(self, source):
        source_mask = (source!= self.src_pas_idx).unsqueeze(1).unsqueeze(2)
